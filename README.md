...
# 📝 Labo 01 : Prise en Main de Git pour MLOps
...
Initialisation d'un nouveau dépôt Git local.

* **Commande :** `git init`
* **Preuve / Capture d'écran :**
    ![Étape 1 - Initialisation Git](screenshots/210053.png) 
    
    > *(Source : Screenshot_20251214_210053.png)*

---

### Étape 2 : Premier commit du projet MLOps

Ajout des fichiers initiaux de la structure du projet et création du premier commit.

* **Commandes :** `git add .` puis `git commit -m "feat(project): initial mlops lab structure"`
* **Preuve / Capture d'écran :**
    ![Étape 2 - Premier Commit](screenshots/210343.png) 
    
    > *(Source : Screenshot_20251214_210343.png)*

---

### Étape 3 : Observer une modification avec `git diff`

Modification du fichier `src/monitor_drift.py` (changement de seuil de 2.5 à 2.0) et observation des changements avec `git diff` avant de commiter.

* **Observation (`git diff`) et Commit :**
    
    ![Étape 3a - Git Diff](screenshots/210549.png) 
    > *(Source : Screenshot_20251214_210549.png)*

    ![Étape 3b - Commit de l'ajustement](screenshots/210616.png)
    > *(Source : Screenshot_20251214_210616.png)*

---

### Étape 4 : Créer une branche de fonctionnalité liée au lab

Création et développement de la branche `feature/api-request-id`.

* **Commandes :** `git checkout -b feature/api-request-id` et `git commit -m "feat(api): improve request_id handling"`
* **Preuve / Capture d'écran :**
    ![Étape 4 - Création et Commit sur la Feature Branch](screenshots/211053.png) 
    
    > *(Source : Screenshot_20251214_211053.png)*

---

### Étape 5 : Fusionner la branche feature dans la branche principale

Fusion de `feature/api-request-id` dans `master` (*fast-forward merge*).

* **Commandes :** `git switch master` et `git merge feature/api-request-id`
* **Preuve / Capture d'écran :**
    ![Étape 5 - Fusion et Log](screenshots/211155.png) 
    
    > *(Source : Screenshot_20251214_211155.png)*

---

### Étape 6 : Créer un conflit de merge sur src/train.py

Création de la branche `feature/change-gate`, commit sur cette branche et sur `master` (pour diverger), puis résolution du conflit de fusion.

* **Processus de Conflit et Résolution :**
    
    ![Étape 6 - Conflit et Résolution](screenshots/211505.png) 
    
    > *(Source : Screenshot_20251214_211505.png)*

---

### Étape 7 : Utiliser `git stash` dans le contexte du lab

Sauvegarde temporaire des modifications sur `src/rollback.py` (`git stash`) et restauration (`git stash apply`).

* **Stash et Apply :**
    
    ![Étape 7 - Git Stash](screenshots/211652.png) 
    
    > *(Source : Screenshot_20251214_211652.png)*

---

### Étape 8 : Tester `git reset` sur un fichier d'expérimentation

Création de plusieurs versions d'un fichier d'expérience (`experiments/reset_test.txt`) pour tester `git reset` (les commits préparatoires sont affichés).

* **Création des Commits d'Expérimentation :**
    
    ![Étape 8 - Commits Préparatoires au Reset](screenshots/211843.png) 
    
    > *(Source : Screenshot_20251214_211843.png)*

---

### Étape 9 : Annuler un commit avec `git revert`

*(Cette étape n'est pas illustrée par une capture d'écran fournie.)*

* **Principe :** `git revert <hash-du-commit-a-annuler>`
* **Description :** Un nouveau commit est créé qui annule les modifications du commit cible.
...
* **Principe :** `git checkout feature/branch-a` puis `git rebase master`
* **Description :** Cette opération réécrit l'historique de la branche feature pour la placer à jour par rapport à `master`, créant un historique linéaire et propre.
